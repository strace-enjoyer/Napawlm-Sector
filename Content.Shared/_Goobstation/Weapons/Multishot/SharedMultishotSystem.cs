...
public sealed class SharedMultishotSystem : EntitySystem
{
    ...
    // [Dependency] private readonly MissChanceSystem _miss = default!; you cant miss bru
    ...
    private void OnRequestShoot(RequestShootEvent msg, EntitySessionEventArgs args)
    {
        ...
        foreach(var gun in gunsEnumerator)
        {
            ...
            if (target != null)
            {
            _gunSystem.AttemptShoot(user.Value, gunEnt, gunComp, shootCoords, target.Value);
            }
            else //i have 0 idea why i did this in the first place, it works, sure? but couldnt i just take out target from the original????? it works so i wont change it i guess...........
            {
    ...
    private void OnAmmoShot(EntityUid uid, MultishotComponent comp, ref AmmoShotEvent args)
    {
        ...
        // args.FiredProjectiles.ForEach(ent => _miss.ApplyMissChance(ent, comp.MissChance)); //do not make me mad
    }

    // private void DealStaminaDamage(EntityUid weapon, MultishotComponent component, EntityUid target) nope
    // {
    ...
    // }
    //
    // private void DamageHands(EntityUid weapon, MultishotComponent component, EntityUid target) //why
    // {
    ...
    // private void OnExamined(Entity<MultishotComponent> ent, ref ExaminedEvent args) //* Napawlm, you cant miss bra
    // {
